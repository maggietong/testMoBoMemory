...
void new_error(unsigned long addr, unsigned long max_error, char *trigger)
{
	...
	if (devs[0] == NULL) {
		lprintf(LOG_WARN, "No memory found for address %lx", addr);
		exit(1);
	}
	for (i = 0; devs[i]; i++) {
		...
		if (val == max_error) { 
			lprintf(LOG_WARN, "Large number of corrected errors in memory at %s", loc);
			lprintf(LOG_WARN, "Consider replacing it");
