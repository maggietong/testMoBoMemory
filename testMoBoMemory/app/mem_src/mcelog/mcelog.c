...
int process(int fd, unsigned recordlen, unsigned loglen, char *buf);

static void disclaimer(void)
{
	lprintf(LOG_WARN, "---------------------------------------------------------------");
	lprintf(LOG_WARN, "Hardware event. This is not a software error.");
}
...
{ 
	...
	lprintf(LOG_WARN, "TSC %llx %s", tsc, ret >= 0 && buf ? buf : "");
	free(buf);
}
...
static void dump_mce(struct mce *m, unsigned recordlen) 
{
	...
	/* should not happen */
	if (!m->finished)
		lprintf(LOG_WARN, "not finished?");
	lprintf(LOG_WARN, "CPU %d %s ", cpu, bankname(m->bank));
	if (m->tsc)
		print_tsc(cpu, m->tsc, m->time);
//	lprintf(LOG_WARN, "");
	if (m->ip)
		lprintf(LOG_WARN, "RIP%s %02x:%llx", 
		       !(m->mcgstatus & MCG_STATUS_EIPV) ? " !INEXACT!" : "",
		       m->cs, m->ip);
	n = 0;
	if (m->status & MCI_STATUS_MISCV) {
//		n += lprintf(LOG_WARN, "MISC %llx ", m->misc);
		lprintf(LOG_WARN, "MISC %llx ", m->misc);
        n = n+1;
    }
	if (m->status & MCI_STATUS_ADDRV) {
//		n += lprintf(LOG_WARN, "ADDR %llx ", m->addr);		
		lprintf(LOG_WARN, "ADDR %llx ", m->addr);
        n = n+1;
    }
//	if (n > 0)
//		lprintf(LOG_WARN, "");
	if (m->time) {
		time_t t = m->time;
		lprintf_n(LOG_WARN, "TIME %llu %s", m->time, ctime(&t));
	} 
	switch (cputype) { 
	case CPU_K8:
...
//		Wprintf("\n");
	...
	if (recordlen >= offsetof(struct mce, cpuid) && m->cpuid) {
		...
		lprintf(LOG_WARN, "CPUID Vendor %s Family %u Model %u",
...
static void dump_mce_raw_ascii(struct mce *m, unsigned recordlen)
{
	...
	if (!m->finished)
		lprintf(LOG_WARN, "not finished?");
	lprintf(LOG_WARN, "CPU %u", m->extcpu ? m->extcpu : m->cpu);
	lprintf(LOG_WARN, "BANK %d", m->bank);
	lprintf(LOG_WARN, "TSC %#llx", m->tsc);
	lprintf(LOG_WARN, "RIP %#02x:%#llx", m->cs, m->ip);
	lprintf(LOG_WARN, "MISC %#llx", m->misc);
	lprintf(LOG_WARN, "ADDR %#llx", m->addr);
	lprintf(LOG_WARN, "STATUS %#llx", m->status);
	lprintf(LOG_WARN, "MCGSTATUS %#llx", m->mcgstatus);
	if (recordlen >= offsetof(struct mce, cpuid))
		lprintf(LOG_WARN, "PROCESSOR %u:%#x", m->cpuvendor, m->cpuid);
#define CPRINT(str, field) 				\
	if (recordlen >= offsetof(struct mce, field))	\
        lprintf(LOG_WARN, str, m->field)
	//	Wprintf(str "\n", m->field)
...
{	
	...
	if (recordlen == 0) {
		lprintf(LOG_INFO, "no data in mce record");
		return 0;
	}

	len = read(fd, buf, recordlen * loglen); 
	if (len < 0) {
		lperror(LOG_ERR, "mcelog read"); 
		return errno;
	}
    ...
    global_record_errors += (len / (int)recordlen);
    lprintf(LOG_INFO, "This time recorded errors [%d]", (len/(int)recordlen));
    lprintf(LOG_WARN, "The total recorded errors [%d]", global_record_errors);
	...
	if (recordlen > sizeof(struct mce))  {
		lprintf(LOG_WARN, "warning: %lu bytes ignored in each record\n",
				(unsigned long)recordlen - sizeof(struct mce)); 
		lprintf(LOG_WARN, "consider an update\n"); 
	}

	if (finish)
		return 0;
	
	return (len/(int)recordlen);
}
...
{
//    Wprintf("The total recorded error count is %d\n", global_record_errors);
//    Wprintf("Exit MCE child\n");
}
*/
static void signal_int_exit(int sig)
{
    lprintf(LOG_INFO, "Exit MCE child");
    lprintf(LOG_INFO, "Memory test total MCE errors");
    lprintf(LOG_INFO, "      total        [%d]", global_record_errors);
    if (global_record_errors != 0)
        fprintf(stdout, "----------Warning: MCE errors happened, please check MCE error messages----------\n");
...
{
	struct mcefd_data d = {};
    struct mce_global_ctrl_msrs global_msrs = {};
    struct mce_bank_msrs bank_msrs = {};
    unsigned long corre_errors = 0;
    int ret = 0;
    int fd = 0;
    char *logfn = LOG_DEV_FILENAME;
    sigset_t mask;
    ...
    signal(SIGABRT, signal_int_exit);
    }

    fd = open(logfn, O_RDONLY);
    if (fd < 0) {
        lperror(LOG_ERR, "Cannot open '%s' device file", logfn);
        return errno;
    }
     
    if (ioctl(fd, MCE_GET_RECORD_LEN, &d.recordlen) < 0) {
        lperror(LOG_ERR, "ioctl for MCE_GET_RECORD_LEN command is failure");
        return errno;
    }
    if (ioctl(fd, MCE_GET_LOG_LEN, &d.loglen) < 0) {
        lperror(LOG_ERR, "ioctl for MCE_GET_LOG_LEN command is failure");
        return errno;
    }
...
#if 0
    if (ioctl(fd, MCE_GET_GLOBAL_CTRL_MSRs,  &global_msrs)) {
        lperror(LOG_ERR, "ioctl for MCE_GET_GLOBAL_CTRL_MSRs is failure");
        return errno;
    }
    
    bank_msrs.bank = 8;  /* Bank 8 shows memory mce information  */
    if (ioctl(fd, MCE_GET_BANK_MSRs, &bank_msrs)) {
        lperror(LOG_ERR, "ioctl for MCE_GET_BANK_MSRs is failure");
        return errno;
    }
    /* Mask except 52~38 bits   */
    corre_errors = 0;
    corre_errors = bank_msrs.IA32_MCi_STATUS & 0x1FFFC000000000; 
    /* 52~38 CORE_ERR_CNT is corrected error count  */
    corre_errors = corre_errors >> 38;  
    *msr_errors = corre_errors;
    lprintf(LOG_NOTICE, "      MSR        [%d]", corre_errors);
#endif
	if (flag == GET_MCE) {
  		*record_errors = process(fd, d.recordlen, d.loglen, d.buf);
//    lprintf(LOG_NOTICE, "      Records    [%d]", *record_errors);
	} else if (flag == MCE_LOOP) {
		check_cpu();
//		prefill_memdb();
		if (!do_dmi)
			closedmi();
		...
		event_signal(0);
		*record_errors = poll_loop(exitonerror);	
	}
