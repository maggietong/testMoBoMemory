...
#define BUS_PP_SHIFT     0x9
	...
	if (mca & (1UL << 12)) {
		lprintf(LOG_WARN, "corrected filtering (some unreported errors in same region)");
		mca &= ~(1UL << 12);
	}

	if (mca < NELE(msg)) {
		lprintf(LOG_WARN, "%s", msg[mca]); 
		return;
	}

	if ((mca >> 2) == 3) { 
		lprintf(LOG_WARN, "%s Generic memory hierarchy error", get_LL_str(mca & 3));
	} else if (test_prefix(4, mca)) {
		lprintf(LOG_WARN, "%s TLB %s Error",
		...
		lprintf(LOG_WARN, "%s CACHE %s %s Error", type, level,
		...
		if (mca == 0x400)
			lprintf(LOG_WARN, "Internal Timer error");
		else
			lprintf(LOG_WARN, "Internal unclassified error: %x", mca & 0xffff);
	} else if (test_prefix(11, mca)) {
		lprintf(LOG_WARN, "BUS %s %s %s %s %s Error",
		...
		*ismemerr = 1;
		...
		lprintf(LOG_WARN, "Unknown Error %x", mca);
}
...
"to uncorrected errors soon",
		[3] ="res3" };
	if (track) {
		lprintf(LOG_WARN, "Threshold based error status: %s", msg[track]);
	}
}
		       ...
		       int socket)
{
	...
	lprintf_n(LOG_WARN, "MCi status:");
	if (!(status & MCI_STATUS_VAL))
		lprintf(LOG_WARN,"Machine check not valid");

	if (status & MCI_STATUS_OVER)
		lprintf(LOG_WARN, "Error overflow");
	
	if (status & MCI_STATUS_UC) 
		lprintf(LOG_WARN, "Uncorrected error");
	else
		lprintf(LOG_WARN, "Corrected error");

	if (status & MCI_STATUS_EN)
		lprintf(LOG_WARN, "Error enabled");

	if (status & MCI_STATUS_MISCV) 
		lprintf(LOG_WARN, "MCi_MISC register valid");

	if (status & MCI_STATUS_ADDRV)
		lprintf(LOG_WARN, "MCi_ADDR register valid");

	if (status & MCI_STATUS_PCC)
		lprintf(LOG_WARN, "Processor context corrupt");

	if (status & (MCI_STATUS_S|MCI_STATUS_AR))
		lprintf(LOG_WARN, "%s", arstate[(status >> 55) & 3]);

	if ((mcgcap == 0 || (mcgcap & MCG_TES_P)) && !(status & MCI_STATUS_UC)) {
		track = (status >> 53) & 3;
		decode_tracking(track);
	}
	lprintf(LOG_WARN, "MCA: ");
	decode_mca(status & 0xffffL, track, cpu, ismemerr, socket);
}

static void decode_mcg(__u64 mcgstatus)
{
	lprintf_n(LOG_WARN, "MCG status:");
	if (mcgstatus & MCG_STATUS_RIPV)
		lprintf(LOG_WARN, "RIPV ");
	if (mcgstatus & MCG_STATUS_EIPV)
		lprintf(LOG_WARN, "EIPV ");
	if (mcgstatus & MCG_STATUS_MCIP)
		lprintf(LOG_WARN, "MCIP ");
	lprintf(LOG_WARN, "");
}
