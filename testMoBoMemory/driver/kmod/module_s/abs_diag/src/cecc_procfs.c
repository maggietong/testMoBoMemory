...
int cecc_procfs_init(void)
{
	int ret = -ENOMEM;

	if (NULL == g_proc) {
		g_proc = create_proc_entry(ABS_DIAG_CECC_PROC_FILENAME, 0644, NULL);
		if (NULL != g_proc) {
...
void cecc_procfs_clean(void)
{
	...
	*/
	if (NULL != g_proc) {
		remove_proc_entry(ABS_DIAG_CECC_PROC_FILENAME, NULL);
		g_proc = NULL;
	}
}

void cecc_procfs_add(void)
{
